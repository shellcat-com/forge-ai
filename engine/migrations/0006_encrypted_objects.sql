-- Published encrypted object storage with a bounded hosted capacity policy.
-- Does not enable source adoption, a worker, or retention policy by itself.
BEGIN;
...
-- Serialized global capacity includes orphan/retired ciphertext. Purging releases
-- bytes only; permanent object identities remain counted to prevent unbounded
-- tombstones and accidental identity reuse. No runtime can raise these limits.
CREATE TABLE forge_objects.capacity (
 singleton boolean PRIMARY KEY CHECK(singleton),
 max_bytes bigint NOT NULL CHECK(max_bytes BETWEEN 1 AND 67108864),
 max_objects integer NOT NULL CHECK(max_objects BETWEEN 1 AND 10000),
 used_bytes bigint NOT NULL DEFAULT 0 CHECK(used_bytes>=0 AND used_bytes<=max_bytes),
 used_objects integer NOT NULL DEFAULT 0 CHECK(used_objects>=0 AND used_objects<=max_objects)
);
REVOKE ALL ON forge_objects.capacity FROM PUBLIC;
GRANT SELECT,UPDATE(used_bytes,used_objects) ON forge_objects.capacity TO forge_object_guard;
GRANT forge_object_guard TO CURRENT_USER;
GRANT CREATE ON SCHEMA forge_objects TO forge_object_guard;
CREATE OR REPLACE FUNCTION forge_objects.before_create() RETURNS trigger
 LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,forge_objects AS $$ BEGIN
 PERFORM forge_objects.lock_key(NEW.object_key);
 IF NOT EXISTS(SELECT 1 FROM forge_objects.policy WHERE singleton) THEN RAISE EXCEPTION 'object retention policy unavailable'; END IF;
 UPDATE forge_objects.capacity SET used_bytes=used_bytes+octet_length(NEW.ciphertext),used_objects=used_objects+1
 WHERE singleton AND used_bytes+octet_length(NEW.ciphertext)<=max_bytes AND used_objects<max_objects;
 IF NOT FOUND THEN RAISE EXCEPTION 'OBJECT_CAPACITY_UNAVAILABLE'; END IF;
 RETURN NEW;
END $$;
CREATE FUNCTION forge_objects.after_purge() RETURNS trigger
 LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,forge_objects AS $$ BEGIN
 IF OLD.ciphertext IS NOT NULL AND NEW.ciphertext IS NULL THEN
  UPDATE forge_objects.capacity SET used_bytes=used_bytes-octet_length(OLD.ciphertext) WHERE singleton;
  IF NOT FOUND THEN RAISE EXCEPTION 'object capacity unavailable'; END IF;
 END IF;
 RETURN NEW;
END $$;
CREATE TRIGGER release_ciphertext_capacity AFTER UPDATE OF ciphertext ON forge_objects.versions
 FOR EACH ROW EXECUTE FUNCTION forge_objects.after_purge();
ALTER FUNCTION forge_objects.before_create() OWNER TO forge_object_guard;
ALTER FUNCTION forge_objects.after_purge() OWNER TO forge_object_guard;
REVOKE ALL ON FUNCTION forge_objects.before_create(),forge_objects.after_purge() FROM PUBLIC;
REVOKE CREATE ON SCHEMA forge_objects FROM forge_object_guard;
REVOKE forge_object_guard FROM CURRENT_USER;
COMMIT;
