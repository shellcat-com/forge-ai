...
it('classifies generated JSON validation failures without exposing provider text',async()=>{
 const original=globalThis.fetch;
 globalThis.fetch=async()=>new Response(JSON.stringify({error:{code:'json_validate_failed',message:'private failed generation'}}),{status:400});
 try {const {request}=await import('../../src/server/providers/transport');await expect(request('https://api.groq.com/openai/v1/chat/completions',{},new AbortController().signal,0)).rejects.toMatchObject({code:'malformed'});}finally{globalThis.fetch=original;}
});
