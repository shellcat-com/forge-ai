...
describe('registered preview fixtures', () => {
  let preparedReady: Awaited<ReturnType<typeof h.ready>>
  beforeEach(async () => {
    // Keep the full source/build fixture setup outside the security assertions'
    // existing deadline. This still traverses the same E2 engine for every case.
    preparedReady = await h.ready()
  })
  it('consumes one ticket atomically under 16 concurrent requests; stores only hashes', async () => {
    const p = preparedReady,
  ...
  describe('two completed fixture applications', () => {
    let p: Awaited<ReturnType<typeof h.ready>>, other: Awaited<ReturnType<typeof h.ready>>
    beforeEach(async () => {
      // Complete both full E2 fixtures in setup. The security operation/assertion
      // deadline remains unchanged and no authorization or pipeline step is skipped.
      p = preparedReady
  ...
  it.each([
    ...
    switch (kind) {
      case 'membership':
        await h.db.admin.query('DELETE FROM forge_control.memberships WHERE user_id=$1', [
          p.actor.id,
        ])
        break
      case 'logout':
      ...
      case 'disabled-workspace':
        await h.db.admin.query(
          'UPDATE forge_control.workspaces SET disabled_at=now() WHERE id=$1',
          [p.actor.workspace]
        )
        break
      case 'deletion':
  ...
  describe('completed fixture job cleanup', () => {
    let p: Awaited<ReturnType<typeof h.ready>>
    beforeEach(async () => {
      p = preparedReady
    ...
    it('revocation schedules canonical resource teardown and cleanup preserves hostname tombstone', async () => {
      ...
      const state = await h.db.admin.query(
        'SELECT state FROM forge_control.environments WHERE id=$1',
        [p.route.environment_id]
      )
...
describe('unregistered preview fixture', () => {
  let p: Awaited<ReturnType<typeof h.ready>>
  beforeEach(async () => {
    p = await h.ready('late.vercel.app', false)
  })
  it('late worker registration racing revocation never resurrects a revoked preview', async () => {
    ...
    let revoked = false
    const revocation = h.preview
      .revoke(p.actor.token, p.actor.csrf, p.route.id, p.host)
      .then(() => {
        revoked = true
      })
    try {
