...
export async function request(
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if(response.status===400){
      // Read a bounded machine code only. Never expose provider messages or failed generations.
      let code:unknown;
      try {const body=await boundedJson(response,300000);code=(body as {error?:{code?:unknown}})?.error?.code;}catch{/* Unknown errors remain configuration failures. */}
      if(code==='json_validate_failed'||code==='tool_use_failed')throw new ProviderError('malformed');
      if(code==='context_length_exceeded')throw new ProviderError('limit');
