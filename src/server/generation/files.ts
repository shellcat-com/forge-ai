...
export const safePath = (path: string) =>
  ...
  /^app\/(page\.tsx|globals\.css|components\/[a-zA-Z0-9_-]+\.tsx|api\/[a-zA-Z0-9_/-]+\/route\.ts)$/.test(
    path,
  );
...
export function validateFiles(files: FileMap): FileMap {
  if (
    Object.keys(files).length > 40 ||
    Buffer.byteLength(JSON.stringify(files)) > 256000
  )
