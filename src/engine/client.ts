...
const validatePath = (path: string) => {
  if (!path.startsWith('/') || path.startsWith('//') || /[\\\r\n\0]/.test(path)
    || path.includes('..') || /%2f|%5c|%2e/i.test(path)) throw new Error('INVALID_ENGINE_PATH')
  return `/api/v1${path}`
}
export class EngineClient {
  ...
  private async request(path: string, init: RequestInit, limit = 256 * 1024): Promise<unknown> {
    ...
    const body = await boundedResponse(response, limit, init.signal)
    let result: unknown
  ...
  async read<T>(path: string, decode: (input: unknown) => T, signal?: AbortSignal): Promise<T> {
    return decode(await this.request(path, { method: 'GET', signal: requestDeadline(signal) }))
  }
