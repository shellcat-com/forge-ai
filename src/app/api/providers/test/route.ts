...
export async function POST(request: Request) {
  ...
  if (process.env.FORGE_AUTH_MODE === 'hosted')
    return Response.json(
      { error: 'Provider diagnostics are available only to local operators.' },
      { status: 403 }
    )
  ...
  try {
    ...
    return Response.json({ error: 'Invalid provider selection' }, { status: 400 })
  }
  if (busy) return Response.json({ error: 'A provider test is already running.' }, { status: 409 })
  busy = true
  ...
  const stream = new ReadableStream({
    async start(sink) {
      ...
      const send = (event: unknown) => {
        if (!signal.aborted) sink.enqueue(encoder.encode(JSON.stringify(event) + '\n'))
      }
      try {
