...
try {
  ...
  await run('node', ['node_modules/next/dist/bin/next', 'build', '--webpack'], 180000)
  const tests = existsSync('lib/generated/tests')
    ? readdirSync('lib/generated/tests')
        .filter((name) => /^[a-zA-Z0-9_-]+\.ts$/.test(name))
        .map((name) => 'lib/generated/tests/' + name)
    : []
  if (!tests.length) throw new Error('Application acceptance tests are required.')
  console.log('Running application tests inside the isolated runtime.')
  await run(
    'node',
