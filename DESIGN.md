...
## Hosted model connections
When the hosted identity bridge is configured, Connections shows owner-scoped provider keys with masked entry fields, explicit save/check/select states, replacement and confirmed removal. Never persist keys in browser storage or return them in API responses. A successful metadata check is labeled separately from generation verification. List only reviewed model candidates and require confirmation of free-tier access before checking selection. Missing server configuration retains the explicit unavailable state. Use existing semantic tokens, wrapped forms, labeled controls, visible focus and 44px targets.
