...
try {
  const files = {
    ...
    'app/globals.css': await readFile('templates/next-app/app/globals.css', 'utf8'),
  }
  handle = await runtime.create(files, null, (text) => process.stdout.write(text))
  ...
  assert.ok(
    !inspect.Config.Env.some((v: string) => /^(.*API_KEY|DATABASE_URL|FORGE_BYOK_KEYRING)=/.test(v))
  )
  for (const target of [
    'https://example.com',
  ...
  const version = await runtime.version(handle)
  assert.match(version.postgresImage ?? '', /^sha256:/)
  const databaseInspect = JSON.parse(await docker(['inspect', handle.name + '-db']))[0]
  assert.equal(databaseInspect.HostConfig.ReadonlyRootfs, true)
  assert.equal(databaseInspect.Config.User, 'postgres')
  assert.deepEqual(databaseInspect.HostConfig.PortBindings, {})
  assert.equal(databaseInspect.Mounts.length, 0)
  await runtime.destroy(handle)
  handle = undefined
  handle = await runtime.create(files, snapshot, () => {}, version)
  const items = await (await fetch(`http://127.0.0.1:${handle.port}/api/items`)).json()
  assert.equal(items[0].title, 'Snapshot evidence')
  console.log(
    'PASS: offline install, build, HTTP preview, PostgreSQL CRUD, snapshot/restore, resource settings, no secrets, blocked egress/host access.'
  )
} finally {
