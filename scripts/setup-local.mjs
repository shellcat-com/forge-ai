...
function docker(args, env = process.env) {
  ...
  if (result.status !== 0)
    throw new Error(
      "Docker setup failed. Check Docker Desktop and port 55432.",
    );
  return result.stdout.trim();
}
if (
  existsSync(".env.local") &&
  /^DATABASE_URL=/m.test(readFileSync(".env.local", "utf8"))
) {
  ...
  console.log(
    "Created dedicated local PostgreSQL. Configuration stored privately in .env.local.",
  );
}
...
for (let attempt = 0; attempt < 30; attempt++) {
  ...
  if (attempt === 29)
    throw new Error("PostgreSQL did not become ready. Inspect Docker Desktop.");
