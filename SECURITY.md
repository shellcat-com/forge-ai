...
The checked-in application is a static client. It makes no model requests, stores no projects, and executes no generated code. The container hardening limits the web server, not untrusted workloads. See [docs/security-model.md](docs/security-model.md) for the current and target threat models.
