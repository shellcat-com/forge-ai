...
| `node scripts/publishing/check-public.mjs EVIDENCE --stage STAGE` | PASS six fresh Chromium contexts: light/dark390/768/1440, public assets/fonts, same-origin GET-only requests, no API request, API404, navigation, keyboard, no page errors/overflow. All six screenshots visually inspected. Canonical Next.js `/hosting` production route also passed six fresh browser cases (content/navigation/reflow, no page errors). |
...
Read-only Vercel preflight verified the authenticated CLI account and sole accessible team `biswas07` (`team_redhKy5rKgFelsT5oaWR7nYv`), active Hobby plan. Existing `parable`, `biswas-khatiwada-portfolio`, `hatch` and `parcel-zeta-silk` were audited without changes; none links the Forge Git repository. Proposed dedicated `forge-ai-demo` and `forge-generated-portfolio` names returned404. Existing website-capacity authorization applies; no paid services/upgrades/domain or model spend authorized. New dedicated project `forge-ai-demo` (`prj_5NmNEwVtJcH2BhGcPPpe5OTzo1GL`) created after rechecking name absence. Its authenticated metadata confirms no Git link, env variables, server framework or install/build command. No unrelated project changed.

This is a public unavailable frontend, **not a working hosted builder or generated portfolio**. No portfolio deployment ID/source hash can be provided yet; none was fabricated. No provider/control/deployment secret enters the static build/runtime. Deployment credentials are held only by the trusted operator CLI. Production: [forge-ai-demo-omega.vercel.app](https://forge-ai-demo-omega.vercel.app). Immutable deployment: [forge-ai-demo-kcmp4ynqr-biswas07.vercel.app](https://forge-ai-demo-kcmp4ynqr-biswas07.vercel.app). Deployment ID `dpl_HQkCUrvZnLVXx4LjGfo64qUu1jh3`; source commit `339fb8e44234cca9ec5cfe3590e62709458e40a4`. Static digest `9c32175e25d487270efc35b55864fc85e52cef2d47896c53b35f863d604d9098`. All34 files matched exact SHA256/length over unauthenticated requests on both production and immutable URLs. Six fresh production Chromium cases passed; no backend/external requests, cookies, page errors or asset failures were observed. Known deployment credential representations were absent from the exact public bytes. These checks are observed supporting evidence, not proof against arbitrary unknown obfuscation.

Deployment command: `vercel deploy --prebuilt --prod --yes --scope biswas07 --meta forgeSourceCommit=339fb8e44234cca9ec5cfe3590e62709458e40a4`, from the audited private stage. Vercel reports Ready and a0ms prebuilt build; `vercel inspect --logs` contains no build-command log because no remote source build ran. Exact CLI, inspect, config/receipt, URL/hash and browser records are in `docs/reports/evidence/task-08/`. First deployment: no prior rollback target; remove only this exact deployment if withdrawing, or reproduce the same reviewed commit/receipt. No project deletion performed. See runbook for future prior-version rollback.

Draft PR: [#9](https://github.com/shellcat-com/forge-ai/pull/9). Code commit CI PASS: [GitHub Actions34437475628](https://github.com/shellcat-com/forge-ai/actions/runs/34437475628), Node24.20.0 with a fresh dependency installation, native PostgreSQL, lint/typecheck/tests/build/control constraints and standalone control build. This provides clean CI reproduction of the code commit, distinct from local shared-dependency checks. Native UI control attempt reported Chrome unavailable through CUA, so native browser zoom and OS reduced motion remain unverified.
...
Task 01 may integrate owned implementation and metadata changes from this draft PR. Task 09 received the immutable URL/source/static hashes for independent validation. No shared live adapter was bypassed or duplicated. Exact generated source/job/template identities remain absent because no actual model-backed portfolio run occurred.

Final local cleanup: temporary canonical Next.js listener stopped; only Task08 .next output removed to release disk. The shared frozen dependency tree and other task outputs remain untouched.
