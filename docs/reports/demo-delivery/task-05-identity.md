...
| Final root-issuer compatibility regression | PASS: 26/26 OIDC protocol tests, targeted lint/strict TS | [JSON](../evidence/task-05/root-issuer-tests.json), [log](../evidence/task-05/root-issuer-tests.log); preserves exact root issuer without requiring a trailing slash |
...
Initial PR CI run [34437628422](https://github.com/shellcat-com/forge-ai/actions/runs/34437628422) passed clean install, lint, typecheck, tests, build and native checks on Node 24.20.0 for commit `c4236ba025fa53583db71b1c1e1b4c87de552664`; the final root-issuer fix triggers a separate final-head CI run. Task 07 separately passed an actual native preview-grant/consume/renewal-versus-operator race against that operator revision; its task-owned evidence remains synthetic and does not close live handoff.
