...
| Python simulations | 18 passed: launch intent before spawn, delayed launch failure/tombstones, stop-before-create, attempt adoption, expiry/epoch rejection, unsafe cleanup retention, systemd mask/kill ordering, no-NIC launch plan, hard quota argv, watchdog fencing, RPC MAC/replay, hostile paths/commands, credential separation, exact prepared cache/archive comparison and malformed cache rejection. No privileged OS operations. |
