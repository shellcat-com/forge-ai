...
## Final independent audit and handoff

Task09 final report commit **`c839aebbf2dc7b3c7c8a987a788f91896fb48344`** is preserved in authored merge **`dfbc8b8`**. I verified all10 hashes in its [final audit manifest](../evidence/task-09/combined-audit-manifest.json); retained audit records show529/529 historical/current worker comparisons,48/48 new coordinator artifacts and59/59 combined source hashes. Its [report](task-09-acceptance.md#final-source-delta-and-retained-regression-evidence) independently confirms final sourceCI, preserves exact historical bindings and identifies coordinator-run tests separately. Added-file scanner findings are two verified coordinator-artifact hashes plus the two already reviewed report prose/Git arguments; no credential identified. Final-delta evidence scan had zero findings. Source/config whitespace and local report-link checks pass; historical raw evidence whitespace remains unchanged.

Final source [CI34439679978](https://github.com/shellcat-com/forge-ai/actions/runs/34439679978) passed on88a5a39, and report/evidence headf510a5f passed [CI34439844957](https://github.com/shellcat-com/forge-ai/actions/runs/34439844957). Later commits contain this audit/report only; current-head CI remains visible on draftPR8 and is checked at handoff. No need to rerun unchanged local suites for report-only additions.

Independent package audit at88a5a39 found778 entries and an old allowlist missing14 new runtime files. It remains unapplied; npm omits the root lockfile, so this tarball is not claimed as an `npm ci` source distribution. MIT is preserved. Historical personal paths, archive/media rights and dependency compatibility require a separate reviewed distribution decision. No package was published and no Git history rewritten.

**Deliverable:** a published, reviewable **draft PR8 against master**, with the canonical reconciliation, completed worker foundations, tested source88a5a39, evidence and explicit remaining dependencies. The board is the current ownership/allocation authority. Existing branches and authors remain intact; shared dirty checkout remains untouched; no second team, force-push, PR merge action or automatic merge to master occurred.

**Still open:** the shared identity/lifecycle, global accounting/cleanup, real worker adoption/authority/live-stage and canonical0004–7 composition described on the board; real Linux image/external-check qualification; actual services/private ingress and named operators; exact provider entitlement/prices and explicit numeric budget; released portfolio profile plus a real generated source/export/publication. Those are unfinished implementation and live-input dependencies, not passing acceptance. Vercel currently serves only the truthful availability page. Public multi-user generation is unapproved; live model spend remains$0. Baseline delivery does not establish first live generated-demo, private-alpha or production readiness.


## Release verification follow-up

A new user-started release/self-host Task09 (`01a08a27-e72b-7073-955e-b2a5518ed8f5`) starts from frozen published baselineefa13a9. Added the [release coordination board](../../operations/forge-release-coordination.md), with exact ownership, UI-role assignment pending, preserved migration allocations and **no final release-staged target yet**. The new brief requires connected public Forge and separate generated-app in-product Publish; it does not permit a portfolio/availability-page substitution or relaxed sandbox/preview gates.

Reviewed and corrected stale shared [self-hosting](../../self-hosting.md) and [provider integration](../../provider-integration.md) descriptions against `scripts/setup-local.mjs`, `worker/index.ts`, `src/server/providers/registry.ts`, Groq/OpenRouter adapters and current control configuration. They now recognize existing persistence/worker/adapters while separating keyless local pages, legacy Docker development and the unavailable production RFC composition. No runtime/credential/environment/UI code changed, no Docker/generated workload or live API test ran. Source88a5a39 and existing evidence remain intact. Documentation whitespace/link checks apply; prior full/native/browser results are not relabeled as new tests.

RFC0001 §0.1 records the new connected-Forge/in-product-Publish scope and distinguishes provider usage controls from a billing product. Section2 no longer incorrectly limits public publishing to the portfolio. Publication profiles, database hosting and lifecycle contracts remain unresolved; this is a scope amendment, not release acceptance or new infrastructure/spending authority.

New release Task09 also reported misleading `/app` legacy worker readiness guidance. Task01 confirmed its source (`src/server/projects/service.ts:239`, rendered by `src/components/workspace.tsx`) and recorded an open release UI finding for the not-yet-identified new Task04 role. No UI/source fix or new browser execution is claimed. The new Task09 PR handoff is review-only; no automatic integration/merge is requested. Its pending report is distinct from the earlier integrated demo Task09 audit.
