...
## Canonical demo-delivery composition

[Task 01 coordination](operations/demo-delivery-coordination.md) and [RFC 0001 §0](rfcs/0001-app-generation-vertical-slice.md) supersede earlier scope deferrals. Preserve the current Next.js application and local worker while integrating the RFC durable control/source flow through explicit adapters. The preserved `engine/integration` source bridge and `src/engine` review client remain synthetic/unrouted until the shared identity, accounting, runtime and API seams are implemented. Do not map IDs or run both workflows against the same project implicitly.

BYOK and a real public portfolio demonstration are included. Vercel hosts both websites without a domain purchase. Public hosting, private preview authorization, isolated execution, persistent workers/data and public multi-user generation have separate acceptance and configuration. The default-off laboratory remains unavailable to public traffic.
