...
| --- | --- | --- |
| Identity — 01/05 | Exact HTTPS origin; Better Auth secret reference; selected Google/GitHub OAuth client or verified email transport; approved invitations and two synthetic test users | `FORGE_AUTH_MODE=hosted` selects application sessions, not a completed RFC identity bridge. Native OIDC/CSRF/role tests use synthetic issuer data. |
