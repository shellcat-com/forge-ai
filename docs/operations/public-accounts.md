...
The resulting `local-website-attempt.webm` is explicitly labeled **local workflow regression, synthetic account/mail, website build unavailable**. It follows signup/login with a simple bakery website request, the actual unavailable response, Connections, draft recovery, logout and password recovery. Playwright records the page viewport, not browser chrome or the mailbox. All accounts/passwords are synthetic; token-bearing navigation is absent from video chrome. Inspect the output before publishing it. The full live generation-to-publication demonstration cannot be replaced by this recording. The earlier `local-auth-regression.webm` remains historical evidence of the account milestone.
