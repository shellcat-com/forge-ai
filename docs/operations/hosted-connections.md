...
`node --import tsx scripts/install-hosted-control.ts` installs the reviewed application/control migrations and limited logins in a dedicated hosted PostgreSQL database. Supply `FORGE_MIGRATION_DATABASE_URL`, `FORGE_MIGRATION_DATABASE_HOST` and the matching `FORGE_MIGRATION_DATABASE_NAME` through protected configuration. The URL accepts only supported `sslmode` options; the connector always verifies the TLS certificate and hostname. Do not disable certificate verification to work around connection errors.

For initial runtime creation, set `FORGE_INSTALL_SECRET_OUTPUT` to a new file within the ignored `.private` directory. The installer refuses to overwrite secrets or reset an existing login. It creates four independent restricted runtime credentials with a connection limit, grants only the corresponding auth/API/worker/maintenance role, and saves their URLs in a mode-0600 file. Protect this file like server secrets. It contains no provider key and must not be committed. The migration owner is never a runtime credential.

Set `FORGE_INSTALL_SCHEMA_ONLY=true` for subsequent schema/hash verification or additive installation without replacing runtime credentials. Each SQL change and its receipt commit together; a failure preserves already committed migration receipts and stops. The script preserves existing application migration hashes and does not edit previously published SQL. Database-level PUBLIC CONNECT/TEMP privileges are revoked only in the explicitly selected dedicated database; intended runtime groups receive CONNECT. Preserve the dedicated database and credentials during code rollback.
