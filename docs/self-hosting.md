...
Use Node 24.20.0 and npm. Run `npm ci`, `npm run verify`, then `npm start`. Forge listens on 127.0.0.1:3000. Public pages render without a provider key. Project persistence requires migrated PostgreSQL; generation additionally requires a configured provider, Docker image and running worker.
